...
from .broker_base import (
    BrokerError, InsufficientFundsError, OrderRequest, OrderSide, OrderStatus, OrderType, ProductType,
)
...
async def get_engine(uid: str) -> ExecutionEngine:
    return await get_live_engine(uid)
...
async def place_simple_order(
    ...
    order = OrderRequest(
        ...
        order_type=order_type, limit_price=limit_price, product_type=product_type,
    )
    ...
    trade_id = save_trade_execution(uid, {
        ...
        "limit_price": limit_price, "product_type": product_type.value, "mode": "LIVE",
...
async def get_funds_dict(uid: str) -> dict:
    ...
    return {
        "ok": True, "mode": "LIVE", "broker": engine.broker.name,
